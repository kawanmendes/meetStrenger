...
export function AnimalAvatar({ source, animal = 'MS', size = 104, style, isLogo = false }: AnimalAvatarProps) {
  ...
  const styles = useMemo(() => StyleSheet.create({
    avatar: {
      ...
      backgroundColor: isLogo ? 'transparent' : 'rgba(255,255,255,0.88)',
      borderWidth: isLogo ? 0 : 2,
      borderColor: isLogo ? 'transparent' : 'rgba(255,255,255,0.72)',
      overflow: 'hidden',
      ...(isLogo ? {} : getShadow(theme.shadows.glass)),
    },
    image: {
      width: isLogo ? '100%' : '78%',
